...
export default function DisplayChat(props) {
  return (
    <div className="card">
        ...
        <div className="chat-nav card-header">
      ...
      <form className="input-group input-message" onSubmit={props.handleSendMessage}>
        <input 
        type="text" 
        class="form-control shadow-none" 
        placeholder="Type a meassage" 
        aria-label="Recipient's username" 
        aria-describedby="basic-addon2" value={props.message}
        onChange={props.handleInputBoxChange}/>
        <div class="input-group-append">
        <button className="btn btn-outline-secondary shadow-none" type="submit">Send</button>
        </div>
      </form>     
    </div>
  );
